...
func (rm *responseModifier) WriteHeader(code int) {
	rm.Header().Set("Access-Control-Allow-Origin", rm.Config.AllowedOrigin)
	rm.Header().Set("Access-Control-Allow-Credentials", rm.Config.AllowCredentials)
...
func main() {
	...
	port := ":4000"
	if config.ListenOn != nil {
		port = *config.ListenOn
	}
...
func describeProxyConfig(config Config) {
	...
	fmt.Println(red("\nProxy Configuration:"))
	fmt.Printf("Target: %s\n", cyan(config.Target))
	fmt.Printf("Allowed Origin: %s\n", cyan(config.AllowedOrigin))
	fmt.Printf("Allow Credentials: %s\n", cyan(config.AllowCredentials))
	fmt.Printf("Port: %s\n", cyan(config.Port))
